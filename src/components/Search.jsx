...
const Search = () => {
  return (
    <div
      className="p-2 pd-5 bg-white rounded-md md:rounded-full flex-col 
    ...
    >
      ...
      <Select>
        ...
        <SelectContent>
          {Data.instType.map((Instrument, index) => (
            <SelectItem value={Instrument.name}>{Instrument.name}</SelectItem>
          ))}
        </SelectContent>
      </Select>
      <Separator orientation="vertical " className="hidden md:block" />
      ...
      <Select>
        ...
        <SelectContent>
          {Data.Pricing.map((Prices, index) => (
            <SelectItem value={Prices.amount}>{Prices.amount}</SelectItem>
          ))}
        </SelectContent>
      </Select>
      <div>
